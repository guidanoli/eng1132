...
Conversely, calling Python objects in Lua expose some interesing differences between the two.

* Python methods are bound to instances, while Lua methods aren't.
* Python function calls accept keyword arguments, while Lua functions don't.

For calling Python objects in Lua, the `__call` metamethod is triggered. The callee and arguments in the stack are
converted to Python objects and the call is executed in a `try-except` block in case any Python exception is raised.

First, we need to address the problem with Python methods. We would like to translate this:

```lua
callee:function(...) == callee.function(callee, ...)
```

... into this:

```python
callee.function(...) == callee_type.function(callee, ...)
```

We basically need to check if the callee and the first argument are equal and if the method belongs to the callee type.
In that case, we simply convert the method into a class function.

We also need to find a way to call Python objects with keyword arguments in Lua.
The solution we came up with envolved allowing the user to construct a userdatum containing positional and keyword arguments
...
* The sequential part of the table (keys are integers from 1 to its length) is converted to positional arguments
* The record part of the table (keys are strings) is converted to keyword arguments
* Any key that does not fit in any of the previous two criterium is invalid

An interesting consequence of this heuristic is that non-sequential integer keys are considered invalid.
